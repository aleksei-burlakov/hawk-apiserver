...
function generateResourceBtnGroup(role, targetRole, allowPromoteDemote, name, type, active
    ...
    <li>
        <a
            href="/cib/live/primitives/${name}/edit" class="edit"
        >
    ...
    <div class="btn-group" role="group">
        ...
        <a
            class="demote btn btn-default btn-xs ${allowPromoteDemote ? 'visible' : 'hidden' }"
...
function getCrmResource(resourceID, crmResources){
...
function generateNodeBtnGroup(nodeID, nodeName, crmResources, cibResources, fenceHistory) {
    const nodeEvents = [];
    // select events on nodeName
    for(const cibResource of cibResources ?? []) {
        const crmResource = getCrmResource(cibResource.ID, crmResources);
        for(const event of cibResource.Events ?? []) {
            if(event.OnNode == nodeName) {
                nodeEvents.push({
                    ...event,
                    ResourceID: cibResource.ID, // all event fields + ResourceID
                    ResourceRole: crmResource.Role,
                    ResourceTargetRole: crmResource.TargetRole,
                    ResourceMaintenance: crmResource.Maintenance,
                    ResourceManaged: crmResource.Managed
                });
            }
        }
    }
...
function generateResourceRow(role, targetRole, name, location, type, active, maintenance, managed, nodeNames, cibResource, isClone, cloneResourceID, expanded) {
    ...
    if (isClone) {
        const btnGroupSub = generateResourceBtnGroup(role
            , targetRole
            , false /*thumb-up/down only in the main row*/
            , cloneResourceID
            , type
            , active
            , isMaintenance
...
function populateNodesTab(nodes, crmResources, cibResources){
    ...
    for (const node of nodes ?? []) {
        ...
        const btnGroup = generateNodeBtnGroup(node.ID, node.Name, crmResources, cibResources, node.fenceHistory);
...
async function updateClusterStatus() {
    ...
    populateNodesTab(Nodes, Resources, CibResources);
}
